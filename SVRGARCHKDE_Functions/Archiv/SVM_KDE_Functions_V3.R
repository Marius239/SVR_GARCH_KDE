svm_kde_dopar <- function(q, block, test, variance_mdl, var_q){
  ...
  results <- foreach(i = loop_start:loop_end, .combine = "rbind") %dopar% {
    ...
    variance.mdl <- variance_mdl #"ARMA"
    
    svm_kde(x = x, y = y, q = q, var_q = var_q, variance.mdl = variance.mdl, kernel = "gaussian")
  ...
  results <- plotResults(results, allData, test_n)
...
svm_kde <- function(x, y, q, var_q, variance.mdl, kernel){
  ...
  # Set paramters
  svm_var_eps       <- quantile(scale(eps)^2, var_q)
  svm_var_cost      <- 10^(-1)
  sigma             <- 0.02
  ...
  if(variance.mdl == "AR")   fcast_vola <- svm_var$fitted
  if(variance.mdl == "ARMA") fcast_vola <- svm_var$model$fitted
  ...
  if(variance.mdl == "AR")   vola_new_dat <- u[nrow(u),1]
  if(variance.mdl == "ARMA") vola_new_dat <- cbind(u[nrow(u),1], svm_var$lagRes[length(svm_var$lagRes)]) 
  
  if(variance.mdl == "AR")   vola_pred  <- predict(svm_var, newdata = vola_new_dat)
  if(variance.mdl == "ARMA") vola_pred  <- predict(svm_var$model, newdata = vola_new_dat)
  ...
  if(variance.mdl == "AR")   u_sc <- eps[-1]/sqrt(fcast_vola)
  if(variance.mdl == "ARMA") u_sc <- eps[-(1:2)]/sqrt(fcast_vola)
...
plotResults <- function(results, allData, test_n){
  ...
  plot(results$True, type = "p", ylim = c(ylim_min, ylim_max), 
       ylab = "Return in Percent", main = "99.5% and 0.5% VaR-Forecast for NYSE")
